...
Page({
  ...
  getOrderInfo() {
    let that = this;
    let postmsg = {
      status: that.data.status?that.data.status:0,
      business_id : that.data.business_id,
    }
    wx.showLoading({
      title: '订单加载中...',
    })
    common.get("/business/shangjiaOrder", postmsg).then( res => {
  ...
  getMeConcern() {
    ...
    common.get('/coupon/pay_my_coupon',{
      business_id: that.data.business_id,
    }).then(res => {
      wx.hideLoading();
      if(res.data.code == 200){
        ...
        if(data.length > 0){
          data.forEach(ele => {
            ele.h_num = ( Number(ele.stock_total) - Number(ele.stock) )
          });
        }
        if(data.length > 0){
  ...
  getlistData(){ // 前端实现一次获取总数据后分页获取数据
    ...
    if(that.data.currentTab == '1'){
      that.setData({
        // 将新获取的数据拼接到之前的数组中
        couponlists: that.data.couponlists.concat(list[page]),
      })
    }
  },
