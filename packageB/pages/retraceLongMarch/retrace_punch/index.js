...
Page({
  ...
  onLoad: function (options) {
    let that = this;
    if(options.newplayInfo){
      let newplayInfo = JSON.parse(options.newplayInfo);
      that.setData({
        newplayInfo,
        id: newplayInfo.id,
        exhibitAddress: newplayInfo.name,
      })
    }
    // 实例化API核心类
  ...
  addDynamic(o){
    ...
    common.post('/long_march_roule/add_dynamic',{
      member_id: wx.getStorageSync('member_id'),
      images_list:JSON.stringify(o), //发圈图片
      contents: that.data.textareaVal,
      lat: that.data.lat,
      lng: that.data.lng,
      punch_in_address_id: that.data.newplayInfo.id
    }).then(res =>{
      wx.hideLoading();
