...
Page({
  ...
  data: {
    ...
    beusedList:[],
    dealType:'buy',  //  订单类型 buy: 我买的 sell: 我卖的
    list: [],
    pageIndex: 1,
    pageSize: 15,
    hasMore: true,
  ...
  onLoad: function (options) {
    ...
    if(options.express_status){
      that.setData({
        express_status: options.express_status,
      })
    }
    that.getbeusedList();
  },
  ...
  getbeusedList(){
    ...
    let p = {
      ...
      express_status: that.data.express_status
    };
    
    if(that.data.dealType == 'sell'){
      p.work_member_id = wx.getStorageSync('member_id');
    }
    if(that.data.dealType == 'buy'){
      p.member_id = wx.getStorageSync('member_id');
    }
    wx.showLoading({
      title: '加载中...',
    })
  ...
  cancelOrder(e){
    ...
    wx.showModal({
      ...
      success:function(res){
        if(res.confirm){
          //删除订单
          common.get("/life/index?op=order_cancel", {
