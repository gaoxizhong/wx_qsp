...
Page({
  ...
  onLoad: function (options) {
    console.log(options)
    let that = this;
    let pay_money = options.pay_money;
    that.setData({
      member_id: wx.getStorageSync('member_id'),
      all_integral: options.all_integral,
      pay_money: pay_money,
      number: options.number,
      phone: options.phone,

    })
    let book_info = wx.getStorageSync('book_info'); //获取选中的商品对象
    that.setData({
      book_info: book_info,
    })
  ...
  confirmBuy() {
    ...
    wx.showModal({
      title: "",
      content: "【邻居的书】功能为促成短距离（邻里之间）的图书流转，交书方式为按照彼此约定时间与地点进行图书送达，您确定购买吗？",
      success(res) {
        if (res.confirm) {
          wx.showLoading({
            title: '正在加载...',
          })
          common.post("/library/to_order", param).then(res => {
            console.log(res)
            if (res.data.code == 200) {
              wx.hideLoading()
              if (res.data.data.data != '') {
                ...
                wx.requestPayment({
                  ...
                  success: function (res) {
                    ...
                    setTimeout(function () {
                      wx.navigateTo({
                        url: '/packageA/pages/library/mySubmit/index?library_phone=' + phone,
                      })
                    }, 1000)
                    return;
                  },
